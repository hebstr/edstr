...
- Auto-exclusion in `edstr_extract()` scans every distinct matched text
  ...
  anchors). The scan runs on every call, whatever `exclus_manual` and
  `exclus_auto_token_min` are set to, so the cost is not opt-in;
  `exclus_auto_escape` only shrinks the rows it scans. Watch for
  slowdowns on large corpora.
- At the default `exclus_auto_token_min = 10`, that scan’s output is
  then discarded in full: auto-exclusions only apply to n-grams larger
  than the threshold, and no realistic `token` value reaches 11. Lower
  the threshold to make the heuristic fire.
