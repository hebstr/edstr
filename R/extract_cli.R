.extract_cli <- \(data,
  ...
  cli_ul()
    cli_li("Total: {nrow(data_total)}")
    cli_li("Final: {nrow(data)} ({cli_p_final})")
    cli_end()
  cli_text("\n\n")
  cli_alert_info("{.strong Matching}")
  cli_ul()
    cli_li("Total: {nrow(data_match)} from {cli_n_join} {id} ({cli_p_join} final obs.)")
  ...
  cli_ul()
    ...
    cli_li("Duplicates: {cli_n_dupl}")
    cli_li("Over {nchar_max} chr.: {nrow(data_exclus_nchar)}")
    cli_end()

  cli_text("\n\n")
  cli_rule()
  cli_text("\n\n")
  cli_alert_success("{nrow(data_id)} matchs from:")
  cli_ul()
    cli_li("{cli_n_extract} {id} ({cli_p_extract} final obs.)")
    cli_li("{cli_n_group} {group}")
    cli_end()
  cli_text("\n\n")
  cli_alert_success("{nrow(data_count)} distinct expressions from {cli_n_concept} concepts")
